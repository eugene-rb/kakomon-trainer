...
自動設定は、採点プロバイダの選択にかかわらず、設定済みの Anthropic APIキー・モデルとウェブ検索を使用します。問題構成、設問種別、配点、採点形式を取得し、出典・確信度・配点合計の不一致を表示します。非公表の配点は推定です。結果を確認し、解答領域を指定して「保存」で確定してください。既存設問がある場合は、全置換または不足分のみ追加を選べます。追加では既存設問の配点・領域を維持します。

配点原則は次回の調査の参考として保存します。取得した解答解説の要約は、ローカルの解答解説がない場合の採点補助に使用します。これらは `%LOCALAPPDATA%/KakomonTrainer/` に保存され、設定画面の「AIの学習キャッシュを削除」から削除できます。

「問題別採点基準を調査」では、既存の設問IDと内容をもとに当該年度の解説記事を検索し、正答の骨子・必須要素・部分点・誤答や減点の観点を問題別に整理します。出典URL、対象年度、確信度、公式基準と推定の区別を記録し、一般的な配点原則とは別の `exam-explanations.json` に大学・年度・文理・科目・設問IDで保存します。「選択設問の採点基準」で内容と出典を確認・コピーできます。設問構成や解答領域は変更しません。対応する解説が見つからない設問の基準は更新しません。

問題別の基準は記述・画像採点の際に該当設問分だけ参照します。一般原則より設問固有の要件を優先し、明示的なローカルの正答・採点基準があればそれを優先します。別年度・別文理・別設問の基準へのフォールバックは行いません。旧版の短い解説メモも保持しますが、問題別基準として利用するには再調査してください。
...
./scripts/build_windows.ps1 -Version 2.2.0
```
